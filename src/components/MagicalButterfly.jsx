 ...
 * MagicalButterfly
 * ─────────────────
 * Wings always flap at a calm constant rate (CSS animation, not scroll-driven).
 * Scroll only controls position + direction the butterfly faces.
 * Movement is clamped — butterfly stops and parks once TeamSection is done.
 *
 * Props
 * ─────
 * scrollYProgress  – MotionValue<0..1> from useScroll() in About page
 * colorTheme       – 'orange' | 'blue'
 */
export default function MagicalButterfly({ scrollYProgress, colorTheme = 'orange' }) {
  const isBlue = colorTheme === 'blue';
  const ct = colorTheme;

  // Clamp: stops well before the footer section border
  const clampedProgress = useTransform(scrollYProgress, (v) => Math.min(v, 0.92));

  // Original zigzag criss-cross path — 12 keyframes, butterflies weave full screen
  // Vertical lanes are offset so they never share the same spot when crossing
  const scrollKeys = [0, 0.12, 0.2, 0.3, 0.42, 0.5, 0.6, 0.72, 0.8, 0.88, 0.9, 0.92];

  // Orange: upper lane — starts right, zigzags left/right, parks bottom-right
  // Blue:   lower lane — starts left, zigzags right/left, parks bottom-left
  // At crossing points the vertical gap is 20%+ so they never overlap
  const leftArr = isBlue
    ? ['8%', '52%', '90%', '90%', '42%', '5%', '5%', '42%', '90%', '90%', '22%', '10%']
    : ['8%', '45%', '8%', '8%', '55%', '92%', '92%', '55%', '8%', '8%', '75%', '88%'];

  const topArr = isBlue
    // Blue lane: 18% → 84%
    ? ['18%', '28%', '34%', '34%', '46%', '58%', '58%', '74%', '82%', '82%', '84%', '84%']
    // Orange lane: 22% → 78%
    : ['22%', '14%', '24%', '24%', '36%', '48%', '48%', '58%', '68%', '68%', '76%', '78%'];

  const leftPos = useTransform(clampedProgress, scrollKeys, leftArr);
  const topPos = useTransform(clampedProgress, scrollKeys, topArr);

  // Original rotation — faces direction of travel through the zigzag
  const rotateArr = isBlue
    ? [20, 145, 15, 15, -160, -15, -15, 160, 15, 15, -150, -20]
    : [-20, -145, -15, -15, 160, 15, 15, -160, -15, -15, 150, 20];

  const rotateZ = useTransform(clampedProgress, scrollKeys, rotateArr);

  const scale = useTransform(clampedProgress, scrollKeys,
    [2.2, 2.8, 1.8, 1.8, 2.8, 1.8, 1.8, 2.8, 1.8, 1.8, 2.4, 2.0]);



  const color1 = isBlue ? '#00A8FF' : '#FF7A00';
  const color2 = isBlue ? '#0055FF' : '#D04500';

  return (
    <Fragment>
      <style>{`
        /* ── Wings always flap at a calm constant speed — NOT tied to scroll ── */
        @keyframes flap-left-${ct} {
          0%, 100% { transform: rotateY(12deg) rotateX(8deg); }
          50%       { transform: rotateY(55deg) rotateX(-4deg); }
        }
        ...
        .bfly-lw-${ct} {
          ...
          /* constant calm flap — 0.9s regardless of scroll */
          animation: flap-left-${ct} 0.9s infinite alternate ease-in-out;
        }
        .bfly-rw-${ct} {
          left: 50%;
          transform-origin: left center;
          animation: flap-right-${ct} 0.9s infinite alternate ease-in-out;
        }

        /* Only show on large screens */
        .bfly-root-${ct} { display: none; }
        @media (min-width: 1024px) { .bfly-root-${ct} { display: block; } }
      `}</style>
      ...
      >
        {/* Body + wings wrapper — gentle float bob */}
        <div
          style={{
            position: 'relative',
            width: 90,
            height: 65,
            transformStyle: 'preserve-3d',
            animation: `float-body-${ct} 2.4s infinite ease-in-out`,
          }}
        >
          {/* ── Left Wing ── */}
